...
try:
    ...
    logger.info("Google ADK loaded successfully for Escalation Agent")
...
class EscalationAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "escalation_agent", config: Dict[str, Any] = None):
        ...
        # Configuration - Lower threshold to catch high severity issues
        self.severity_threshold = self.system_config.get('agents.escalation.severity_threshold', 0.6)
        self.email_delay = self.system_config.get('agents.escalation.email_delay', 300)  # 5 minutes
        # LLM for severity assessment refinement
        self.llm_model = self.system_config.get("llm.models.escalation", "gemini-2.0-flash")

        # Initialize statistics early to avoid attribute errors
        ...
        # Initialize Google ADK components for severity assessment
        if self.llm_model and ADK_AVAILABLE:
            self.session_service = InMemorySessionService()
            self.session_id = "escalation_session"
            self.user_id = "system_user"

            # Create ADK LlmAgent for severity assessment
            self.adk_agent = LlmAgent(
                name="severity_assessor",
                model=self.llm_model,
                instruction=self._get_severity_instruction(),
                description="Assesses the severity level of support requests for escalation decisions"
            )
        ...
        self.urgency_patterns = {
            ...
            'business_impact': [
                r'revenue',
                r'customers? (affected|impacted)',
                r'production (down|issue)',
                r'system (down|outage)',
                r'critical (system|service)'
            ],
            'escalation_triggers': [
                r'legal',
                r'lawsuit',
    ...
    async def _ensure_session_initialized(self):
        ...
        if self.adk_agent and not self._session_initialized:
            try:
    ...
    def _get_severity_instruction(self) -> str:
        """Get instruction for the ADK severity assessment agent."""
        return """You are a severity assessment agent for a customer support escalation system. Your role is to:

1. Analyze support messages to determine their severity level
2. Identify urgency indicators and critical keywords
3. Assess the potential business impact
4. Provide structured severity assessment with reasoning

Severity Levels:
- **CRITICAL**: System outages, security breaches, legal issues, revenue-impacting problems
- **HIGH**: Important deadlines, priority issues, significant user impact
- **MEDIUM**: Standard problems, moderate urgency, routine issues
- **LOW**: General questions, minor issues, non-urgent requests

Assessment Criteria:
- Presence of urgency keywords (urgent, critical, emergency, etc.)
- Time sensitivity indicators (deadlines, immediate needs)
...
- "severity_score": A number from 0.0 to 1.0
- "requires_escalation": Boolean indicating if escalation is needed
- "trigger_keywords": List of keywords that influenced the assessment
- "urgency_indicators": List of urgency signals found
- "reasoning": Detailed explanation of the severity assessment

Input: Support message content
Output: Structured severity assessment in JSON format"""
    ...
    async def _assess_severity(self, message: Message) -> SeverityAssessment:
        ...
        # Optionally refine with Google ADK if configured
        try:
            if self.adk_agent is not None:
                llm_eval = await self._adk_refine_severity(original_content, severity_level, severity_score, reasoning_parts)
    ...
    async def _adk_refine_severity(self, original_text: str, base_level: str, base_score: float, reasons: List[str]) -> Optional[Dict[str, Any]]:
        """Use Google ADK to refine severity assessment and return a JSON dict."""
        if self.adk_agent is None:
            return None

        prompt = f"""
Analyze this support message for severity assessment:
...
Please provide a refined severity assessment in JSON format.
"""
        ...
        try:
            ...
            logger.debug(f"ADK refine parse error: {e}")
            return None
